...
from dynasor.evaluator import math_equal, count_not_empty, eqaul_group
...
def openai_chat_completion_stream(client, model, prompt,
                             temperature: float = 0.7,
                             max_tokens: Optional[int] = 2048,  dynasor_saving_effort: tuple = None):
    ...
    if dynasor_saving_effort is not None:
        ...
        for _ in range(0, max_tokens, chunk_size):
            ...
            if not adaptive_end:
                ...
                model=model,
                temperature=0.6,
                prompt=current_prompt + '... Oh, I suddenly got the answer to the whole problem, **Final Answer**\n\n\\[ \\boxed{',
            ...
            current_prompt += result  # Update the prompt with the new text for subsequent iterations
            ...
            if not adaptive_end:
                ...
                probe_certain_count = [not any(word in res.lower() for word in uncertain_words) for res in probe_responses[-threshold:]]
        
                # print("=" * 100)
                # print(probe_text, answer, certain_count)
                # print("=" * 100)
        
            if not adaptive_end and eqaul_group(probe_answers[-threshold:]) and count_not_empty(probe_answers[-threshold:]) == threshold and sum(probe_certain_count) == threshold :
                adaptive_end = True
